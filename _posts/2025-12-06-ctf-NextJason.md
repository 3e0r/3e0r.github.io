...
```
# Суть уязвимости **"JWT algorithm confusion"**

1. Уязвимость в коде:
   - токены задумывались как RS256,
   - но при проверке включили и HS256, при этом:
     - использовали один и тот же ключ (PUBLIC KEY) для обоих алгоритмов,
     - позволили клиенту (атакующему) самому выбирать `alg` в header;
   - для RS256 public key — нормально и безопасно;
   - для HS256 public key превращается в секрет HMAC, а он не секрет.

2. Имея `public.pem`, мы можем:
   - сами выбирать payload (`{"username":"admin"}`);
   - сами подписывать токены (HMAC);
   - полностью обходить бизнес-логику, рассчитанную на «только сервер умеет подписывать».
